...
import { Button, Grid, Textarea, Text, Spacer, Input } from '@nextui-org/react';
...
export default function Step2({txid}: {txid: ethers.providers.TransactionResponse}) {
  ...
  const Receipt = () =>
        <Grid.Container justify='center' alignItems="center" alignContent="center" direction="column" gap={2}>
          ...
          <Spacer y={2} />
          <Textarea aria-label='notes' size="lg" placeholder="Notes (optional)" css={{width: '100%', maxWidth: '570px'}}/>
          <Spacer y={2} />
          <Input required placeholder="VAT" value={0} size="lg" type="number" fullWidth={true} css={{maxWidth: '570px'}} helperText="VAT %" label="VAT" labelRight="%" />
          <Spacer y={2} />
          <Button color="gradient">encrypt for {`${otherPartieAddress?.slice(0,10)}...${otherPartieAddress?.slice(-10,otherPartieAddress.length)}`}</Button>
        </Grid.Container>
